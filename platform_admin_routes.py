...
def get_mayfair_last_run():
    """Last MPL sync row for the chosen tenant. Drives the 'last sync'
    panel on the platform admin page. msr_unmatched carries the rich
    step-4 detail dict; the row is returned as-is (the UI renders it).
    """
    ...
    return jsonify({"last_run": get_last_run(company_id)})
