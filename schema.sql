...
-- ================================================================
-- 27. transcription_hints  (per-company custom vocabulary)
-- ================================================================
-- Words/phrases the transcription engine should listen for. Sent as
-- keyterms_prompt on every transcription request for the owning company.
-- Per-term length is enforced 5-50 chars; case-insensitive uniqueness is
-- enforced in app code (matching the respondents idiom). The 200-term cap
-- is enforced at the API layer using grader.KEYTERMS_PROMPT_MAX_TERMS.
CREATE TABLE transcription_hints (
...
-- are TERMINAL only; intermediate display states (webhook_received,
-- processing, timeout) are derived at poll time from the join with
-- voip_call_queue + interactions.
...
-- ================================================================
-- 33. dashboard_insights  (Sub-Task T — recurring-issues mini-report)
-- ================================================================
-- One row per company. Cached company-wide "what's hurting scores" report
-- generated by Claude Haiku from the last 30 days of rubric weaknesses.
-- Refreshed lazily on a 24h TTL via insights.compute_dashboard_insights_async,
-- or on demand via POST /api/dashboard/insights/refresh.
